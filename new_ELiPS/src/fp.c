...
void fp_mod_printf(char *str, fp_t *A) {
  static fp_t out;
  fp_mod(&out, A);
...
void fp_mod_println(char *str, fp_t *A) {
  static fp_t out;
  fp_mod(&out, A);
  gmp_printf("%s%Nu\n", str, out.x0, FPLIMB);
}
...
void fp_set_ui_to_montgomery(fp_t *ANS, unsigned long int UI) {
  mpn_set_ui(ANS->x0, FPLIMB, UI);
  fp_to_montgomery(ANS->x0, ANS->x0);
}
...
void fp_set_mpn_to_montgomery(fp_t *ANS, mp_limb_t *A) {
...
void fp_set_random_to_montgomery(fp_t *ANS, gmp_randstate_t state) {
  mpz_t tmp;
  mpz_init(tmp);
...
void fp_mulmod(fp_t *ANS, fp_t *A, fp_t *B) {
#ifdef DEBUG_COST_A
  cost_mul++;
  cost_mod++;
#endif
...
void fp_sqrmod(fp_t *ANS, fp_t *A) {
#ifdef DEBUG_COST_A
  cost_sqr++;
  cost_mod++;
#endif
...
void fp_mod_normal(fp_t *ans, mp_limb_t *a, mp_size_t size_a) {
#ifdef DEBUG_COST_A
  cost_mod_nomal++;
#endif
...
void fp_mul_ui_mod_normal(fp_t *ANS, fp_t *A, unsigned long int UI) {
  static mp_limb_t tmp_mul[FPLIMB2];
  mpn_mul_ui(tmp_mul, A->x0, FPLIMB, UI);
  fp_mod_normal(ANS, tmp_mul, FPLIMB2);
}
...
void fp_add_mod(fp_t *ANS, fp_t *A, fp_t *B) {
#ifdef DEBUG_COST_A
  cost_add++;
#endif
#ifdef DEBUG_ASSERT
